$(function () {
  ...
  function renderRecentWorkOrders(assets, workOrders) {
    ...
    $("#recentWorkOrders").html(rows.map(function (item) {
      return '<tr class="' + (App.isActiveUrgent(item) ? "is-urgent" : "") + '"><td>' + App.escapeHtml(item.title) + "</td><td>" + App.escapeHtml(assetMap[item.assetId]) + "</td><td>" + App.priorityBadge(item) + "</td><td>" + App.badge(item.status) + "</td><td>" + App.formatDate(item.requestedAt) + "</td></tr>";
    }).join(""));
  }
